...
class SaltClient:
    ...
    def pillar_fs_path(cls):
        pillar_dirs = cls.master().opts.get('pillar_roots', {'base': []})['base']
        if not pillar_dirs:
            return None
...
class PillarManager:
    ...
    def pillar_installed(cls):
        ...
        if os.path.exists(os.path.join(pillar_base_path, cls.PILLAR_FILE)):
            return True
        try:
            top_data = cls._load_yaml("top.sls")
            if 'base' not in top_data:
                return False
            if 'ceph-salt:member' not in top_data:
                return False
